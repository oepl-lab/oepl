...
export default function ResearchAreaCard({
  ...
  return (
    <div
      ...
      className={`relative rounded-2xl bg-white border border-gray-100 overflow-hidden flex flex-col hover:border-[#E88800]/40 transition-colors card-hover cursor-pointer group ${
        elaExpanded ? "" : "min-h-[280px] md:min-h-[320px]"
      }`}
    >
      {!elaExpanded && (
        <>
      ...
      {elaExpanded && (
        <>
          <div className="absolute top-0 left-0 right-0 h-48 sm:h-52 md:h-60 bg-gray-100 flex items-center justify-center overflow-hidden pointer-events-none">
            {image ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={image} alt="" className="w-full h-full object-contain bg-white" />
            ) : null}
          ...
          <div className="invisible pointer-events-none shrink-0" aria-hidden>
